...
function App({ themes }: { themes: Theme[] }) {
	...
	const { authorizedUser, isAuthenticated, isLoading, error } =
		useWebContext();
	...
	useEffect(() => {
		...
		async function getLikedThemes() {
			...
			if (
				cachedLikedThemes &&
				cacheTime &&
				now - parseInt(cacheTime, 10) < 3600000
			) {
	...
	const filteredThemes = isLoading
		...
		: themesOnly
				.filter(t => {
					...
					const tags =
						filters.length === 0 ||
						filters.every(f => t.tags.includes(f.value));
					return match && tags;
				})
				.sort((a, b) => {
					switch (sort) {
				...
				>
					<div className="flex flex-col items-center">
						<div className="relative inline-flex items-center justify-center">
							<h2 className="text-xl font-semibold mb-3 mt-3 relative z-10">
				...
				<div className="mb-3 mt-8">
					<div className="flex justify-end mb-3">
						...
						<Button
							...
							className="rounded-lg font-medium bg-[#4a63b4] hover:bg-[#3f58a3] text-white"
						>
							{isValid ? (
								<>
				...
				<Tabs defaultValue="themes" className="w-full mt-6">
					...
					<TabsContent value="themes">
						{isLoading ? (
							...
							<ThemeGrid
								likedThemes={likedThemes as any as []}
							...
							<div className="flex items-start gap-3">
								...
								<div>
									...
									<AlertDescription>
										...
										Vencord and may not work with other
										client mods.
									</AlertDescription>
								</div>
							</div>
